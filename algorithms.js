...
// An tree with left and right branches
class Tree {
  constructor(n){
    this.info = n
    this.left = undefined;
  ...
  //returns the information stored in that node
  info(){
    return this.info;
  }
  ...
  //returns true iff current node is a leaf
  isleaf(){
...
// input: ls, an array that transforms
//        tree, a Tree structure
// output: an updated Tree structure with the information from ls
const addTree = function (tree, ls) {
if (ls.length == 0) {
  return []
} else if (ls.length == 2) {
  tree.left = new Tree(ls.shift());
  tree.right = new Tree(ls.shift());
  return []
} else if (ls.length > 2) {
  console.log(ls)
  tree.left = new Tree(ls.shift());
  tree.right = new Tree(ls.shift());
  let ks = addTree(tree.left, ls);
  console.log(ls)
  let ts = addTree(tree.right, ls.slice(2));
  return ts
}
...
//TODO tree search, 
//TODO tree delete, add
//TODO tree sort
// input: tree, an unsorted array
// returns: a list of steps in sorting
const Treesort = function (array) {

  // sort() {
  //   if (this.isleaf()) {
  //     return this
  //   }
  //   if (!this.right.isleaf() && !this.right.isleaf() ) {
  //     this.right = this.sort(this.right);
  //     this.left = this.sort(this.left);
  //     if (this.right.info < this.left.info) {
  //       let temp = this.right;
  //       this.right = this.left;
  //       this.left = temp;
  //     }
  //     if (this.info < this.right.info) {
  //       let temp = this.right.info;
  //       this.right.info = this.info;
  //       this.info = temp; 
  //     } 
